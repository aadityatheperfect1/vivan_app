...
const char VEHICLE_NUMBER[] = "PRQ567";
...
void OnDataRecv(const esp_now_recv_info_t *recv_info, const uint8_t *data, int len) {
  ...
  if (receivedDoc["type"] == "VoiceMessage"){
    serializeJson(receivedDoc, Serial);
    Serial.println();
    return;
  }
...
void sendVoiceChat(const char *macstr, const char *msg) {
...
void TaskSerialReceive(void *pvParameters) {
  while (1) {
    if (Serial.available() > 0) {
      ...
      if (type && strcmp(type, "ChatRequest") == 0) {
      ...
      } else if(type && strcmp(type, "VoiceMessage") == 0) {
        const char *mac = doc["mac"];
        const char *msg = doc["message"];

        if(mac && msg){
          sendVoiceChat(mac,msg);
        }
